...
Factory.prototype.create = function (definition, callback) {
    ...
    async.map(childNodesDefinitions, this.create.bind(this), function(err, results) {
        ...
        self.databaseService.getColumnNames(node._getQuery(), function(err, columnNames) {
            if (err) {
                return callback(err);
            }
            ...
            if (node.shouldCacheQuery()) {
                ...
                self.databaseService.run('select * from ' + targetTableName + ' limit 0', function(err) {
                    if (!err) {
                        return callback(null, node);
                    }

                    var outputQuery = node._getQuery();
