...
function logElapsedTime(logger, nodesToLogByJobId, job) {
    nodesToLogByJobId[job.job_id].forEach(function (node) {
        var queryElapsedTime = getQueryElapsedTime(job.query.query, node.nodeId);
...
function getNodesToLogByJobId(rows) {
    return rows
        ...
        .reduce(function (byJobId, toLog) {
            ...
            byJobId[toLog.jobId].push({ nodeId: toLog.nodeId, nodeType: toLog.nodeType });
