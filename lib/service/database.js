...
DatabaseService.prototype.getElapsedTime = function(jobId, nodeId, callback) {
    this.batchClient.getJob(jobId, function (err, job) {
        if (err) {
            return callback(err);
        }

        if (job.query && job.status === 'done' && Array.isArray(job.query.query)) {
            for (var i = 0; i < job.query.query.length; i++) {

                if (job.query.query[i].query.indexOf('/* camshaft_node_id: ' + nodeId) !== -1) {
                    var start = new Date(job.query.query[i].started_at);
                    var end = new Date(job.query.query[i].ended_at);

                    if (start && end) {
                        var elapsedTimeMilliseconds = end.getTime() - start.getTime();

                        debug('analysis time %j', { analysis: nodeId, time: elapsedTimeMilliseconds });
                        return callback(null, elapsedTimeMilliseconds);
                    }
                }
            }
        }
...
DatabaseService.prototype.queueAnalysisOperations = function(analysis, callback) {
    ...
    this.queryRunner.run(query, function(err, resultSet) {
        ...
        function logElapsedTime(nodeToLog, callback) {
            self.getElapsedTime(nodeToLog.jobId, nodeToLog.nodeId, function (err, elapsedTimeMilliseconds) {
                if (err) {
                    return callback(err);
                }
