...
var NodeSqlDecorator = require('../postgresql/node-sql-decorator');
...
DatabaseService.prototype.run_writable = function(query, callback) {
    this.queryRunner.run(query, QUERY_RUNNER_WRITE_OP, callback);
};

var TIMEOUT_ERROR_MESSAGE = new RegExp('canceling statement due to statement timeout', 'i');

// check err returned by queryRunner operation for a timeout error
DatabaseService.prototype.checkForTimeout = function(err) {
    return !!(err && err.message && err.message.match(TIMEOUT_ERROR_MESSAGE));
};
...
DatabaseService.prototype.createTable = function(node, callback) {
    var sqlDecoratedNode = new NodeSqlDecorator(node);
    this.queryRunner.run(sqlDecoratedNode.createTableQuery(), QUERY_RUNNER_WRITE_OP, function(err, resultSet) {
...
DatabaseService.prototype.createTableIfNotExists = function(node, callback) {
    var self = this;
    this.tableExists(node.getTargetTable(), function(err, tableExists) {
        ...
        self.createTable(node, function(err) {
...
DatabaseService.prototype.registerAnalysisInCatalog = function(analysis, callback) {
    ...
    nodes.forEach(function(node) {
        ...
        var cacheTables = pgArray(
            node.shouldCacheQuery() ? [pgQuoteCastMapper('regclass')(node.getTargetTable())] : [], 'regclass[]'
        );
...
function annotateNodeType(node) {
    return '/* analysis:' + node.getType() + ' */';
}
    ...
    this.getNodesToUpdate(analysis, function(err, nodesToQueueForUpdate) {
        ...
        nodesToQueueForUpdate.forEach(function(nodeToUpdate) {
            if (nodeToUpdate.shouldCacheQuery()) {
                var sqlDecoratedNode = new NodeSqlDecorator(nodeToUpdate);
                ...
                asyncQueries.push({
                    ...
                    query: sqlDecoratedNode.annotatedNodeType() + transactionQuery([
                        sqlDecoratedNode.deleteFromCacheTableQuery(),
                        sqlDecoratedNode.populateCacheTableQuery(),
                        sqlDecoratedNode.checkCacheTableQuery(),
                        sqlDecoratedNode.analyzeTableQuery()
                    ]),
