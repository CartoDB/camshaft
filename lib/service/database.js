...
function DatabaseService(user, dbParams, batchParams, logger) {
    this.user = user;
...
DatabaseService.prototype.setLogger = function (logger) {
    this.logger = logger;
};
...
DatabaseService.prototype.enqueue = function(queries, callback) {
    var self = this;
    this.batchClient.enqueue(queries, function (err, result) {
        if (err) {
            return callback(err);
        }

        if (result && result.job_id) {
            self.logger.info({ job: result.job_id });
        }
...
DatabaseService.prototype.logAnalysisElapsedTime = function (rows) {
    var self = this;
    var logElapsedTime = self.logNodeElapsedTime.bind(self);
    ...
    return rows
        .map(function (row) {
            if (row.last_modified_by) {
                return { nodeId: row.node_id, jobId: row.last_modified_by };
            }
        })
...
DatabaseService.prototype.logNodeElapsedTime = function (nodeToLog, callback) {
    var self = this;

    self.getNodeElapsedTime(nodeToLog.jobId, nodeToLog.nodeId, function (err, elapsedTimeMilliseconds) {
        if (err) {
            return callback(err);
        }
...
DatabaseService.prototype.getNodeElapsedTime = function(jobId, nodeId, callback) {
    this.batchClient.getJob(jobId, function (err, job) {
        if (err) {
            return callback(err);
        }

        if (!hasStartAndEndTime(job)) {
            return callback();
        }
...
function hasStartAndEndTime(job) {
    return job.status === 'done' && job.query && Array.isArray(job.query.query);
}
...
function transactionQuery(queries, nodeId) {
    return ['/* camshaft_node_id: ' + nodeId + ' */ BEGIN'].concat(queries).concat('COMMIT').join(';') + ';';
}
...
DatabaseService.prototype.queueAnalysisOperations = function(analysis, callback) {
    ...
    var query = [
        'SELECT node_id, updated_at, status, last_error_message, last_modified_by',
    ...
    this.queryRunner.run(query, function(err, resultSet) {
        ...
        self.logAnalysisElapsedTime(rows);
