...
var AnalysisLogger = require('./analysis-logger');
...
function DatabaseService(user, dbParams, batchParams, loggerConfig) {
    this.user = user;
    ...
    if (loggerConfig) {
      this.analysisLogger = new AnalysisLogger(loggerConfig, this.batchClient);
    }
}
...
DatabaseService.prototype.queueAnalysisOperations = function(analysis, callback) {
    ...
    this.queryRunner.run(query, function(err, resultSet) {
        ...
        if (self.analysisLogger) {
            // log after this function and scheduled I/O finish
            setImmediate(self.analysisLogger.log.bind(self.analysisLogger, analysis, rows, self.user));
        }
