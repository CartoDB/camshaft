...
var AnalysisTimeLogger = require('./analysis-time-logger');
...
function DatabaseService(user, dbParams, batchParams, logger) {
    ...
    this.logger = logger;
    this.analysisTimeLogger = new AnalysisTimeLogger(this.logger, this.batchClient);
}
...
DatabaseService.prototype.setLogger = function (logger) {
    this.logger = logger;
    this.analysisTimeLogger.setLogger(logger);
};
...
DatabaseService.prototype.enqueue = function(queries, callback) {
    ...
    this.batchClient.enqueue(queries, function (err, result) {
        ...
        if (result && result.job_id) {
            self.logger.info({ job: result.job_id }, 'analysis:job');
        }
...
DatabaseService.prototype.queueAnalysisOperations = function(analysis, callback) {
    ...
    var query = [
        'SELECT node_id, analysis_def::json->>\'type\' as node_type,',
        ' updated_at, status, last_error_message, last_modified_by',
    ...
    this.queryRunner.run(query, function(err, resultSet) {
        ...
        // log after this function and scheduled I/O finish
        setImmediate(self.analysisTimeLogger.log.bind(self, rows));
