...
DatabaseService.prototype.queueAnalysisOperations = function(analysis, callback) {
    ...
    this.queryRunner.run(query, function(err, resultSet) {
        ...
        nodesToQueueForUpdate.forEach(function(nodeToUpdate) {
            ...
            asyncQueries.push({
                query: updateNodeAtAnalysisCatalogQuery(nodeToUpdate.id(), status),
        ...
        nodesToQueueForUpdate.forEach(function(nodeToUpdate) {
            if (nodeToUpdate.shouldCacheQuery()) {
                ...
                asyncQueries.push({
                    query: deleteFromCacheTableQuery(targetTableName),
                    onerror: updateNodeAtAnalysisCatalogQuery(nodeToUpdate.id(), Node.STATUS.FAILED)
                });
                asyncQueries.push({
                ...
                asyncQueries.push({
                    query: populateCacheTableQuery(targetTableName, nodeToUpdate.sql()),
                    onerror: updateNodeAtAnalysisCatalogQuery(nodeToUpdate.id(), Node.STATUS.FAILED)
                });
            }
            asyncQueries.push({
        ...
        if (asyncQueries.hasQueries()) {
            return self.enqueue(asyncQueries.getQueries(), callback);
        } else {
            return callback(null);
        }
    });
