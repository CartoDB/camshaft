...
var getHitsNodeQueryTemplate = dot.template([
    'SELECT hits FROM cdb_analysis_catalog WHERE node_id = \'{{=it._nodeId}}\' limit 1'
].join('\n'));
...
DatabaseService.prototype.trackAnalysis = function(analysis, callback) {
    ...
    async.waterfall([
        function databaseService$trackAnalysis (done) {
            var sql = trackNodeQueryTemplate({ _nodeId: node.id() });

            debug('Tracking analysis with SQL: %s', sql);
            self.queryRunner.run(sql, QUERY_RUNNER_WRITE_OP, function (err, resultSet) {
                debug(err, resultSet);
                return done(err);
            });
        },
        function databaseService$logHits(done) {
            var sql = getHitsNodeQueryTemplate({ _nodeId: node.id() });

            self.queryRunner.run(sql, QUERY_RUNNER_READONLY_OP, function (err, resultSet) {
                if (err) {
                    return done(err);
                }
                self.logger.info({ hits: resultSet.rows[0].hits });
                done();
            });
        }
    ], callback);
