...
function batchErrorMessage(err, response) {
    ...
    if (response.statusCode === 400 &&
        ...
        response.body.error.some(e => e.match(/\bYour payload is too large\b/i))) {
        return 'The analysis SQL code couldn\'t be queued for execution because it is too complex. ' +
            'If you have multiple analyses please try simplifying ' +
            'or materialising some of them to reduce the payload.';
    }
...
BatchClient.prototype.getJob = function (jobId, callback) {
    var requestParams = {
        json: true,
        url: this.endpoint + '/' + jobId,
        qs: {'api_key': this.apiKey},
        headers: {
            'Host': this.hostHeader({username: this.username}),
            'User-Agent': 'camshaft'
    ...
    request.get(requestParams, function(err, response, job) {
        if (err) {
            return callback(err);
        }
        ...
        if (response.statusCode === 400 && response.body.error.indexOf('not found') === -1) {
            return callback();
        }

        if (response.statusCode !== 200) {
            return callback(new Error('Unable to get SQL API batch job'));
        }
