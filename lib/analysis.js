...
AnalysisFactory.prototype.create = function(configuration, definition, callback) {
    ...
    var logger = createLogger(configuration.logger);
    ...
    async.waterfall(
        [
            ...
            function analysis$validate(rootNode, done) {
                logger = logger.child({ analysis: rootNode.id(), username: configuration.user }, true);
            ...
            function analysis$register(analysis, done) {
                analysis.getSortedNodes().map(function (node) {
                    logger.info({ node: node.id(), type: node.getType() }, 'analysis:node-type');
                });
