...
function node2dot(node) {
    ...
    var _node = [
        ...
        '<tr><td port="head" bgcolor="' + d.color + '"><font point-size="16">' + d.type + '</font></td></tr>'
    ];

    Object.keys(d.nodes).forEach(function(nodeKey) {
        _node.push('<tr><td port="' + d.nodes[nodeKey].id() + '"><font color="blue">'+nodeKey+'</font></td></tr>');
    });

    Object.keys(d.attrs).forEach(function(attrKey) {
        _node.push('<tr><td><font color="purple">'+attrKey+'</font>: '+ d.attrs[attrKey]+'</td></tr>');
    });
