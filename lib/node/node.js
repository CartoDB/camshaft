...
Node.prototype.computeRequirements = function(callback) {
    // TODO: by default simply compute maximum of its children
...
Node.prototype.requirementMessages = function() {
    var messages = [];
    if (this.estimated_requirements.result_rows > this.limits.result_rows) {
      messages.push("too many result rows");
    }
    if (this.estimated_requirements.result_size > this.limits.result_size) {
      messages.push("result too large");
    }
    if (this.estimated_requirements.geocoding_calls > this.limits.geocoding_calls) {
      messages.push("too many geocoding calls");
    }
    return messages;
};
