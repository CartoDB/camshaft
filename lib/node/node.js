...
function stringStartsWithDoubleQuotes(string) {
    return string && string.length && string[0] === '"';
}

function addDoubleQuotesToColumnNames(columns, skipTheGeoms) {
    return columns.map(function (columnName) {
        if (skipTheGeoms || SKIP_COLUMNS[columnName]) {
            return columnName;
        }
        return stringStartsWithDoubleQuotes(columnName) ? columnName : `"${columnName}"`;
    });
}

Node.prototype.getColumns = function(skipTheGeoms) {
    skipTheGeoms = skipTheGeoms || false;
    var columns = (skipTheGeoms) ? skipColumns(this.columns) : this.columns;
    
    columns = addDoubleQuotesToColumnNames(columns);
