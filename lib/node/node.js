...
module.exports.PARAM_TYPE = {
    NODE: new Validator(TYPE.NODE, function (node) {
        return !!node.getQuery;
    }),
    NUMERIC: new Validator(TYPE.NUMERIC, Number.isFinite),
    TEXT: new Validator(TYPE.TEXT, function (str) {
    ...
    ENUM: function() {
        var args = (arguments.length === 1 ? [arguments[0]] : Array.apply(null, arguments));
        var expectedType = args.map(JSON.stringify).join(',');
        return new Validator('ENUM(' + expectedType + ')', function(val) {
