...
function NodeValidator(geometry) {
    ...
    this.isValid = function (node) {
        var outputGeometry = node.geometry();
        return !!node.getQuery && (this.geometries.hasOwnProperty(outputGeometry) || outputGeometry === GEOMETRY.ANY);
    };
    this.expects = TYPE.NODE + '(' + this.geometry.join(',') + ')';
}
...
module.exports.create = function createNode(type, expectedParams, output, options) {
    options = options || {};
    function BaseNode() {
    ...
    BaseNode.prototype.validateAndBuild = function() {
        ...
        this.geometry = typeof output === 'function' ? output.bind(this) : function() { return output; };
    };
    return BaseNode;
};
