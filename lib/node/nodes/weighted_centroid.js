...
var TYPE = 'weighted_centroid';

var PARAMS = {
    source : Node.PARAM.NODE(Node.GEOMETRY.POINT),
    weight_col : Node.PARAM.STRING,
    category_col : Node.PARAM.STRING
};
...
var weightedCentroidTemplate = dot.template([
    ...
    'FROM cdb_crankshaft.CDB_WeightedMean(\'{{=it.query}}\',\'{{=it.weight_col}}\',\'{{=it.category_col}}\')'
].join('\n'));
...
WeightedCentroid.prototype.sql = function(){
    return query({
        ...
        weight_col : this.weight_col,
        category_col : this.category_col
    });
};
