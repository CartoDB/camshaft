...
var PARAMS = {
    ...
    input_left_column_join_on: Node.PARAM.STRING(),
    input_left_final_columns: Node.PARAM.ARRAY(),
    input_right: Node.PARAM.NODE(Node.GEOMETRY.ANY),
    input_right_column_join_on: Node.PARAM.STRING(),
    input_right_final_columns: Node.PARAM.ARRAY(),
...
Merge.prototype.sql = function() {
    var leftColumns = setAliasTo(this.input_left_final_columns, INPUT_LEFT_ALIAS);
    var rightColumns = setAliasTo(this.input_right_final_columns, INPUT_RIGHT_ALIAS);
    var finaColumns = leftColumns.concat(rightColumns);

    return query({
        input_left_alias: INPUT_RIGHT_ALIAS,
        input_right_alias: INPUT_LEFT_ALIAS,
        input_left: this.input_left.getQuery(),
        input_left_column_join_on: INPUT_LEFT_ALIAS + '.' + this.input_left_column_join_on,
        input_right: this.input_right.getQuery(),
        input_right_column_join_on: INPUT_RIGHT_ALIAS + '.' + this.input_right_column_join_on,
        final_columns: finaColumns.join(', '),
        left_join: (this.join_type === 'left'),
        geom_from: (this.geom_from === 'input_left') ? INPUT_LEFT_ALIAS : INPUT_RIGHT_ALIAS
    });
};

var queryTemplate = dot.template([
    'SELECT ',
    ' {{=it.geom_from}}.the_geom AS the_geom,',
    ' {{=it.final_columns}}',
    'FROM',
