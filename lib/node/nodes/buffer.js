...
var Buffer = Node.create(TYPE, PARAMS, {
    validate: function (node) {
        if (node.dissolved && (!node.isolines || node.isolines < 2)) {
            throw new Error('Dissolved mode requires a number > 1 for "isolines" param');
        }
    }
});
...
var radiusQueryTpl = dot.template([
    'SELECT',
    '{{=it._columns}}',
...
var isolinesQueryTpl = dot.template([
    'SELECT',
    '{{=it._columns}}',
    'FROM',
...
var isolinesDissolvedTpl = dot.template([
