...
var PARAMS = {
    ...
    weightColunn: Node.PARAM.STRING(),
    weightThreshold: Node.PARAM.NULLABLE(Node.PARAM.NUMBER()),
    popColumn: Node.PARAM.STRING(),
    maxDistance: Node.PARAM.NUMBER(), // meters
    targetID: Node.PARAM.NUMBER() // the "mall" to be studied
};
...
var gravityTemplate = Node.template([
    ...
    ' t as(',
        ...
        ' array_agg(cartodb_id::bigint) as id',
        ' array_agg(the_geom) as g,',
        ' array_agg(coalesce({{=it.w}},0)::numeric) as p',
        ' FROM raw_t',
    ')',
    'SELECT ',
    ...
    ' t,',
    ' s,',
    ' CDB_Gravity(t.id, t.g, t.w, s.id, s.g, s.p,',
    '  {{=it.targetID}}, {{=it.r}}, {{=it.wt}}) g'
].join('\n'));
