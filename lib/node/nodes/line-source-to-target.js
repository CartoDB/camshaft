...
LineSourceToTarget.prototype.checkLimits = function(databaseService, limits, logger, callback) {
    var defaultLimit = null;
    if (!this.closest) {
        // Check and limit the case of all-to-all lines (cartesian product)
        defaultLimit = 1e6;
    }
    var limit = NodeRequirements.getLimit(limits, this.getType(), 'maximumNumberOfRows', defaultLimit);
    NodeRequirements.limitNumberOfRows(this, databaseService, limit, logger, callback);
};
