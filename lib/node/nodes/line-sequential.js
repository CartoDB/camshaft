...
LineSequential.prototype.checkLimits = function(databaseService, limits, logger, callback) {
    var self = this;
    var rowsLimit = NodeRequirements.getLimit(limits, TYPE, 'maximumNumberOfRows', 1000000);
    var pointsLimit = NodeRequirements.getLimit(limits, TYPE, 'maximumNumberOfPointsPerLine', 100000);
    function check(inputRows, numCategories) {
        ...
        return callback(NodeRequirements.nodeError(self, messages));
    }
    NodeRequirements.estimatedNumberOfInputRows(this, databaseService, function(err, inputRows) {
        if (err) {
            if (logger) {
            ...
            if (self.category_column) {
                NodeRequirements.numberOfDistinctValues(
                    self.source, databaseService, self.category_column,
                    function(err, num) {
                        if (err) {
                            if (logger) {
                                logger.logLimitsError(err);
