...
var ttIntersectionTemplate = Node.template([
    ...
    '    \'{{=it.polygons_table}}\',',
    // '    \'@bbox\'::json,',
    '    (select array_to_json(ARRAY[st_xmin(bbox), st_ymin(bbox), st_xmax(bbox), st_ymax(bbox)]) from (select st_extent(the_geom_webmercator) as bbox from {{=it.polygons_table}}) _meta),',
