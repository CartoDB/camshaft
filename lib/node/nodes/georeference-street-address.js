...
GeoreferenceStreetAddress.prototype.getStreetAddressColumnParam = function() {
    ...
    var columns_and_literals = this.street_address_template.trim().split(', ').map(function(fragment) {
        fragment = fragment.trim();
        if(COLUMNS_TEMPLATE_REGEX.test(fragment)) {
            var column_match = COLUMNS_TEMPLATE_REGEX.exec(fragment);
            return column_match[1].trim();
        } else {
            return '\'\'' + fragment + '\'\'';
        }
    });

    if (columns_and_literals.length === 0) {
        return '$tpl$\'' + this.street_address_template + '\'$tpl$';
    }

    return '$tpl$' + columns_and_literals.join(' || \'\', \'\' || ') + '$tpl$';
};
