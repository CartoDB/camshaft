...
var queryTemplate = Node.template([
    ...
    '   SELECT ',
    '      json_build_object(\'id\', cartodb_id, \'address\', {{=it.street_column}}) as data',
    ...
    '   ({{=it.source}}) _x',
    '   LEFT JOIN',
    '   (SELECT (cdb_dataservices_client.cdb_bulk_geocode_street_point(jsonb_agg(data))).* FROM geocoding_data) _y',
    '   USING (cartodb_id) '
].join('\n'));
